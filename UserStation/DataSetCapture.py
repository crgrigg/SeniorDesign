...
def DataSetSave(img):
    path = "C:/Users/rober/OneDrive/Desktop/Images/negative" # This will be the path where your images will be saved If you use the rasberry pi then it will be '/home/pi/desktop/data/images'
    ...
    if saveData:
        blur =501 #cv2.Laplacian(img,cv2.cv_64F).var()  #For blur detection in image
        if count % moduleVal == 0 and blur > minBlur:  # Applying condition for minblur threshold and count divide by module val is 0
            nowTime = time.time()
            cv2.imwrite(path +'/' + str(countSave) + " " +str(int(blur))+" "+str(nowTime)+".png",img)  # This will be the name of image saved in the folder + the extension of image
            countSave += 1
